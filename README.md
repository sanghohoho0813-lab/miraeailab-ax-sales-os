...
| 단계 | 화면 | 내용 |
|---|---|---|
| BEFORE | 미팅 준비 — "업체 정보를 어떻게 가져올까요?" | **[PDF로 1분 준비]**(기업정보·크레탑·회사소개서 PDF → 브라우저에서 텍스트 추출 → **업종·근로자·최근 매출·업력 4가지만** 확인 → 중복 고객 확인 → 저장. 나머지 추출값은 [추출정보 전체보기] 안) · **[30초 빠른 등록]**(회사명 + 클릭 3단계, 연락처 선택[입력/음성/나중에/모름], [음성으로 한 번에 입력], 미팅 시각 [지금·오늘·내일·+30분·+1시간] 실시간 시계) · [기존 고객 선택] |
| BEFORE | 미팅 전략 (한 화면) | 회사 핵심 4가지 → **오늘은 이 3가지만 확인하세요** → 동종업계 실제 사례 최대 2개(추천 이유 포함, 없으면 "사례 없음") → **[미팅 시작]** · 질문은 "오늘 질문 N개 준비됨" 한 줄만 · 상세 전략 / 영업 팁 / 기업자료·관리 / 왜 이렇게 판단했나요? 는 시트로 |
