...
| 단계 | 화면 | 내용 |
|---|---|---|
| BEFORE | 미팅 준비 — "업체 정보를 어떻게 가져올까요?" | **[PDF로 1분 준비]**(기업정보·크레탑·회사소개서 PDF → 브라우저에서 텍스트 추출 → 구조화 → "PDF에서 이 내용을 찾았습니다" 검토[값별 수정/사용 안 함, 페이지·원문 근거] → 중복 고객 확인[기존 고객에 정보 추가: 항목별 기존 유지/PDF 반영] → 저장) · **[30초 빠른 등록]**(회사명 + 클릭 3단계, 대표 연락처 선택[입력/음성/나중에/모름], 항목별 마이크, [음성으로 한 번에 입력] 초안 확인, 미팅 일시 [지금·오늘·내일·+30분·+1시간·다른 날짜] 실시간 시계) · [기존 고객 선택] |
| BEFORE | 미팅 전략 (Strategy Autopilot — 저장 즉시 자동) | "오늘/내일 ○○은 이렇게 접근하세요" · 한 문단 접근법 · 오늘 공략 TOP 3(근거 상태) · [미팅 시작] · 정보 충분도 · [왜 이렇게 판단했나요?](값·출처·페이지) · 실제 사례 ①업종 ②문제구조 ③전환경로(추천 이유) · 오늘 꼭 물어볼 질문 5~8 + 문서→가설→질문 · 고객에게 이렇게 말하세요(OPENING/PRICE/CASE/FUNDING/CLOSING 1~2문장 + 다음 질문) · 주의할 표현 · 추가정보 ≤3 · 기업자료(추출정보 보기 → 사용 안 함 → 즉시 재계산) · 사전진단 |
