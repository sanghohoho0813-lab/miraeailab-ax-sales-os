...
| BEFORE | 미팅 전략 (한 화면) | 회사 핵심 4가지(빈 칸은 `미확인 [선택]`) → **오늘은 이 3가지만 확인하세요** → **[미팅 시작]** → 동종업계 실제 사례 **기본 3개 + [2개 더 보기] (최대 5개 — 10억 이내 조달 3개 이상, 수십억 조달은 2개 이하)**(추천 이유·규모 배지 포함, 없으면 "사례 없음" · **업종을 모르면 엉뚱한 업종 사례를 붙이지 않는다**) → **[미팅 시작]** · 질문은 "오늘 질문 N개 준비됨" 한 줄만 · 상세 전략 / 영업 팁 / 기업자료·관리 / 왜 이렇게 판단했나요? 는 시트로 |
...
| 상시 | **기업분석 도구** (고객 상세 → [기업분석 도구]) | **서류 체크리스트**(기업정보 보고서·사업자등록증·법인등기부·4대보험 명부·재무제표 — 미비 서류를 눈에 띄게, 그 자리에서 올리기) · **기업정보 분석**(크레탑·신용정보 PDF) · **고용지원금 검토**(명부 집계 → 확인할 제도. 금액은 계산하지 않는다) · **정책자금 유력 기관**(371건 사례의 실제 조달 기관 + 한계 고지) |
