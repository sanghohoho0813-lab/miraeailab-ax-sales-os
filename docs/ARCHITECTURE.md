...
> **저장소에 근거가 없는 숫자는 화면에 띄우지 않는다.**

돈이 걸린 도메인(지원금 단가, 자금 한도, 승인 기준)은 근거 없이 지어내면 컨설턴트가 대표 앞에서 틀린 말을 한다.

- 근거가 있으면 근거와 함께 보여 준다 — 정책자금 기관은 "실제 사례 N건" 과 그 사례 링크를 같이 낸다.
- 근거가 없으면 **구조만 만들고 값을 비워 둔다.** `SubsidyCheck.rule = null` 이 그 자리다(규칙표가 들어오면 채운다).
- 데이터의 한계는 화면에 적는다 — "371건 중 203건이 민간투자", "신청 자격이 아니라 실제 사례".
- 테스트로 강제한다: 고용지원금 결과에 금액 모양(숫자+원)이 있으면 실패한다.

## 개인정보 계약 (깨지 말 것)

> **사람 단위 정보는 읽지도 저장하지도 않는다.** 4대보험 명부의 한 줄은 그 자체가 한 사람의 개인정보다.

- 명부 파서는 집계(인원·최근 12개월 입·퇴사)만 남기고 이름·주민번호·생년월일을 남기지 않는다.
- 근거(`sourceText`) 에 명부 원문을 넣지 않는다 — 고정 문구만.
- 값을 읽기 **전에** `scrubPii()` 를 거친다. (읽고 나서 지우는 것이 아니라, 애초에 보지 않는다.)
  이건 원칙일 뿐 아니라 정확성 문제이기도 하다 — 주민번호가 날짜 파싱을 망가뜨린 버그가 실제로 있었다.
- 테스트는 지어낸 입력이 아니라 **브라우저가 실제로 만드는 줄**로 한다. 지어낸 입력은 이 버그를 놓쳤다.

## 첫 화면 계약 (깨지 말 것)

> **그 화면의 주 동작은 첫 화면 안에 있어야 한다.** 정보를 늘리는 변경이 CTA 를 밀어냈다면 그 변경이 잘못된 것이다.

- 화면을 바꾸면 추측하지 말고 **잰다**: `document.scrollHeight / viewport` 와 주 CTA 의 `boundingBox().y / viewport`.
  미팅 전략 화면 기준 CTA 는 1.0 화면 이내(현재 모바일 0.88 · 데스크톱 0.71).
- 목록이 길어지면 기본 개수를 줄이고 [N개 더 보기] 로 연다. 계산은 그대로 두고 **펼침만** 줄인다.
- `sticky` 하단 바 아래에는 바 높이 + offset 만큼 자리를 둔다(spacer). 없으면 문서 끝까지 내려도 마지막 내용이 영구히 가려진다.
- 같은 정보를 두 곳에서 크게 보여 주지 않는다(헤더 시계 ↔ 홈 시계).
