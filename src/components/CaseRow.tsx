 ...
 * 문제 서술·연도는 카드에서 빼거나 아주 작게 둔다. 상세는 [사례 보기] 안에 있다.
 * 조달 규모 배지(10억 이내 / 10~20억 / 수십억 조달)는 남긴다 — 5~30명 회사 대표 앞에서 "우리 규모 얘기인가" 를 한눈에 알아야 한다.
 */
...
import { FUNDING_SCALE_LABEL, fundingScale } from '../engine/caseMatcher'
...
export function CaseRow({ c, reason, onOpen, ctaLabel = '사례 보기', to }: { c: CaseStudy; reason?: string; onOpen?: () => void; ctaLabel?: string; to?: string }) {
  ...
  const scale = fundingScale(c)
  return (
    <article className="lift flex h-full flex-col rounded-(--radius-card) border border-line bg-white p-4" data-testid="case-row" data-scale={scale}>
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
        ...
        <Badge tone={scale === 'small' ? 'ok' : scale === 'large' ? 'warn' : 'neutral'} data-testid="case-scale">
          {FUNDING_SCALE_LABEL[scale]}
        </Badge>
